...
myEditor :: String
myEditor = myTerminal ++ " -e lvim "    -- Sets nvim as editor
...
myShowWNameTheme = def
    { swn_font              = "xft:Iosevka:bold:size=20"
    , swn_fade              = 0.4
    , swn_bgcolor           = "#1c1f24"
    , swn_color             = "#ffffff"
    }
...
myLayoutHook = avoidStruts $ mouseResize $ windowArrange $ T.toggleLayouts monocle
             ...
             where
               myDefaultLayout = withBorder myBorderWidth tall -- noBorders tall
                                 ||| noBorders monocle
...
myManageHook = composeAll
     ...
     , className =? "Xfce4-session-settings"         --> doFloat
...
myKeys =
    ...
    -- KB_GROUP Run Prompt
        , ("M1-<F1>", spawn "rofi -show drun -show-icons -theme 'Pop-Dark' -font 'Iosevka 16'")
